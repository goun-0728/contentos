const DEFAULT_TARGET_URL = 'https://your-project.vercel.app'
...
async function openTargetWithReviews() {
  ...
  chrome.tabs.create({ url: buildImportUrl(settings.targetUrl, pendingReviews) })
...
function downloadReviewsJson() {
  ...
  setStatus('JSON 파일을 다운로드했습니다. 웹앱에서 업로드할 수 있습니다.')
}
...
function buildImportUrl(targetUrl, reviews) {
  ...
  return `${url}#reviews=${encodeURIComponent(JSON.stringify(reviews))}`
}
